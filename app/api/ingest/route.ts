...
export async function POST(request: NextRequest) {
  try {
    ...
    // Support both 'file' (singular from FileUpload) and 'files' (plural)
    let files: File[] = [];
    ...
    return NextResponse.json({
      ...
      ingested,
      warnings,
      summary: `Uploaded ${files.length} file(s), processed ${ingested.length} successfully`,
    });
  } catch (error) {
